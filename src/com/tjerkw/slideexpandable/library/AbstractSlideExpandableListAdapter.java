...
public abstract class AbstractSlideExpandableListAdapter extends WrapperListAdapterImpl {
	 ...
	 * animate expand icon
	 */
	private ImageView expand_icon = null;
	private ImageView expand_icon_hidden = null;
	private ImageView last_open_expand_icon = null;
	private ImageView last_open_expand_icon_hidden = null;
	private View last_toggle_button = null;

	private CourseSylibusFragment courseSylibusFragment = new CourseSylibusFragment();
	...
	/**
	 * Check's if any position is currently Expanded To collapse the open
	 * item @see collapseLastOpen
	 * 
	...
	private void enableFor(final View button, final View target, final int position) {
		...
		button.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(final View view) {
				expand_icon = (ImageView) view
						.findViewWithTag(SaveConstants.context.getResources().getString(R.string.expand_icon));
				expand_icon_hidden = (ImageView) view
						.findViewWithTag(SaveConstants.context.getResources().getString(R.string.expand_icon_hidden));
				...
				if (a != null && a.hasStarted() && !a.hasEnded()) {
					...
					if (type == ExpandCollapseAnimation.EXPAND) {
						if (lastOpenPosition != -1 && lastOpenPosition != position) {
							if (lastOpen != null) {
								System.out.println("====>close opened item and open target item");
								last_toggle_button.setTag("");
								animateView(lastOpen, ExpandCollapseAnimation.COLLAPSE);
								rotateExpandIcon(last_open_expand_icon, last_open_expand_icon_hidden,
										ExpandCollapseAnimation.COLLAPSE);
								notifiyExpandCollapseListener(ExpandCollapseAnimation.COLLAPSE, lastOpen,
										lastOpenPosition);
							}
						...
						last_open_expand_icon = expand_icon;
						last_open_expand_icon_hidden = expand_icon_hidden;
						last_toggle_button = view;
						last_toggle_button.setTag("toggled");
						rotateExpandIcon(last_open_expand_icon, last_open_expand_icon_hidden,
								ExpandCollapseAnimation.EXPAND);
					} else if (lastOpenPosition == position) {
						lastOpenPosition = -1;
						System.out.println("====>close opened item");
						rotateExpandIcon(last_open_expand_icon, last_open_expand_icon_hidden,
								ExpandCollapseAnimation.COLLAPSE);
						last_open_expand_icon = null;
						last_open_expand_icon_hidden = null;
						last_toggle_button.setTag("");
						last_toggle_button = null;
					}
					animateView(target, type);
					notifiyExpandCollapseListener(type, target, position);
					courseSylibusFragment.setItemNum(lastOpenPosition);
				}
			}
		});
	}
	...
	private void rotateExpandIcon(final View target, final View target2, final int type) {
		if (target != null && target2 != null) {
			...
			if (type == ExpandCollapseAnimation.COLLAPSE) {
				target2.startAnimation(animationSet);
			} else {
	...
	public interface SetExpandItemCallBack {
